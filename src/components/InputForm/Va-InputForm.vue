  ...
  <div class="wrapper">
    ...
    <el-col style="padding-left:0">
      ...
      >
        ...
        <el-table-column prop="description" label="表单名称" :show-overflow-tooltip="true" align="center" />
    ...
    <!--添加表单弹窗  -->
    <el-dialog title="添加输入表" :visible.sync="dialogFormVisible" width="500px">
      <el-form ref="ruleForm" :model="ruleForm" label-width="100px" :rules="rules">
        <el-form-item label="表格编号">
          <el-input placeholder="表格编号由系统自动生成" disabled />
        </el-form-item>
        <el-form-item label="表格名称" prop="name" style="margin-top:30px">
    ...
    <!--添加普通字段弹窗 -->
    <el-dialog title="添加字段" :visible.sync="fieldVisible" width="550px" :before-close="handleClose">
      <el-form
        ref="fildForm"
        :model="fildForm"
        :rules="rules"
      ...
      >
        ...
        <el-form-item label="字段描述:" prop="description" style="margin-top:20px">
          <el-input v-model.trim="fildForm.description" />
        </el-form-item>
        <el-form-item label="字段类型:" prop="fldType" style="margin-top:20px">
        ...
        >
          <el-select v-model="fildForm.parentFldId" placeholder="请选择表中表总字段" style="width:100%">
            <el-option v-for="item in tableFields" :key="item.order" :label="item.description" :value="item.order" />
          </el-select>
        </el-form-item>
        <el-form-item label="数据类型:" prop="fldDataType" style="margin-top:20px">
          <el-select v-model="fildForm.fldDataType" placeholder="请选择数据类型" style="width:100%">
            ...
            <el-option v-show="fildForm.fldType === '1' || fildForm.fldType === '7'" label="图片字段" value="9" />
          </el-select>
        </el-form-item>
        <el-form-item label="字段长度:" prop="fldDataLength" style="margin-top:20px">
          <el-input
        ...
        <el-form-item label="字段精度:" prop="fldDataPrecision" style="margin-top:20px">
          <el-input v-model="fildForm.fldDataPrecision" />
        </el-form-item>
        <el-form-item style="margin-top:30px">
          <el-button type="primary" @click="addNow('fildForm')">添加</el-button>
          <el-button @click="resetForm('fildForm')">重置</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    <!-- 字段属性弹窗 -->
    <el-dialog title="字段属性" :visible.sync="showFieldsDetails" width="500px">
      <el-form
        ref="fieldDetails"
        :model="fieldDetails"
        :rules="rules"
      ...
      >
        ...
        <el-form-item label="字段描述:" prop="description" style="margin-top:15px">
          <el-input v-model="fieldDetails.description" disabled />
        </el-form-item>
        <el-form-item label="字段类型:" prop="fldType" style="margin-top:20px">
        ...
        <el-form-item label="数据类型:" prop="fldDataType" style="margin-top:15px">
          <el-select v-model="fieldDetails.fldDataType" :disabled="fieldDetails.fldType !=='1'" style="width:100%">
            <el-option label="字符串字段" value="1" />
            <el-option label="整形字段" value="2" />
            <el-option label="实数字段" value="3" />
            <el-option v-show="fildForm.fldType === '1'" label="布尔字段" value="4" />
        ...
        <el-form-item label="字段长度:" prop="fldDataLength" style="margin-top:15px">
          <el-input
        ...
        <el-form-item label="字段精度:" prop="fldDataPrecision" style="margin-top:15px">
          <el-input v-model="fieldDetails.fldDataPrecision" :disabled="fieldDetails.fldType !=='1'" />
        </el-form-item>
        <el-form-item style="margin-top:20px">
          <el-button type="primary" @click="editForm">立即修改</el-button>
          <el-button @click="resetFieldDetails()">重置</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
// 引入字段操作方法
import { getFormFields, addFormField, deleteFormField, editFormField } from '@/api/input-form/field'
...
export default class extends Vue {
  ...
  transFormData(data:any) {
    ...
    for (const item of data) {
      ...
      if (item['formGroupId']) { // 接收的是输入表单数据
  ...
  // 初始化表单数据
  initForm() {
    if (this.groupId !== null) { // groupId存在，请求表单
      ...
      // })
      this.loading.start()
      getForm(this.groupId as any).then(res => {
        // 1.2 让加载的蒙版消失
        this.loading.close()
        if (res.status === 200) {
  ...
  submitForm(formName:string) {
    (this.$refs[formName] as any).validate((valid:any) => {
      if (valid) { // 表示输入的数据格式符合要求 => 进行保存数据操作
        // 2.1展现正在加载蒙版
        // const loading = this.$loading({
        //   lock: true,
        //   text: 'Loading',
      ...
      if (!this.selectedId) { // 没有选择表单
        ...
        // 4.2- 询问用户是否确定要删除表单
          ...
          // 4.3- 发起删除操作
            ...
            this.loading.start()
            // 4.3.2- 获取需要删除表格的id
            const formId = this.selectedId
            // 4.3.3 -进行删除
            deleForm(formId as number).then(res => {
              this.loading.close()
 ...
 (
  ...
  async addField(row:any) {
    ...
    if (this.fieldDataStorage[formId] !== undefined) { // 已经加载过
  ...
  toAddField(existedField:any, formId:number|string) {
    let currentIndex
    // 判断数组是否为空
    if (existedField.length === 0) { // 创建第一个表单
      currentIndex = 1
    } else { // 不为空
      // 取出最后一个字段fldId的尾数
  ...
  reLoad() {
    ...
    for (const node of this.formLoaded) {
      ...
      this.$set((this.$refs.formList as any).store.states.lazyTreeNodeMap, id, [])
  ...
  removeField(row:any) {
    ...
    this.$confirm('确定要删除该字段吗?', '提示', {
      ...
      // })
      this.loading.start()
      deleteFormField(this.formId as number, [this.fldId]).then(res => {
        if (res.status === 200) {
          this.reLoad()
  ...
  // 修改字段
  editForm() {
    if (this.fieldDetails.fldType === '1') { // 发起修改
